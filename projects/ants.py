...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            if place.ant.is_container and (not self.is_container) and place.ant.can_contain(self):
                place.ant.store_ant(self)
            elif (not place.ant.is_container) and self.is_container and self.can_contain(place.ant):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        a = self.place
        distance = 0
        while a and distance <= self.upper_bound:
            if a.is_hive:
                return None 
            if distance >= self.lower_bound and a.bees:
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 12
        self.throw_at(self.nearest_bee())
        land = self.place.exit
        while land:
            if land.ant:
                if land.ant.is_container and land.ant.ant_contained:
                    land.ant.ant_contained.double()
                land.ant.double()
            land = land.exit
        # END Problem 12
