...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    score0, score1 = 0, 0
    a = strategy(score0, score1)
    while score0 < goal :
        score1 = 0
        while score1 < goal :
            if a != strategy(score0, score1) :
                return False
            score1 += 1
        score0 += 1
    return True 
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    n = 2
    max_index = 1
    max = make_averaged(roll_dice, samples_count)(1, dice)
    while n <= 10 :
        a = make_averaged(roll_dice, samples_count)(n, dice)
        if max < a :
            max = a
            max_index = n
        n += 1
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    if score < opponent_score or score < (GOAL - 20) :
        return sus_strategy(score, opponent_score, 15, 5)
    else :
        a = sus_strategy(score, opponent_score, 10, 5)
        if a > 3 :
            return 3
        else :
            return a
