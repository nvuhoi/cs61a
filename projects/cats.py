...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    lists = [i for i in paragraphs if select(i)]
    if k >= len(lists):
        return ''
    else:
        return lists[k]
...
def accuracy(typed, source):
    ...
    if typed_words == [] and source_words == []:
        ...
        for i in range(len(source_words)):
            if (source_words[i] in typed_words[i]) and len(source_words[i]) == len(typed_words[i]):
                k += 1
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    index_k, k = 0, diff_function(typed_word, word_list[0], limit)
    for i in range(len(word_list)):
        if diff_function(typed_word, word_list[i], limit) < k:
            k = diff_function(typed_word, word_list[i], limit)
            index_k = i
    if k <= limit:
        return word_list[index_k]
    else:
        return typed_word
    # END PROBLEM 5
...
def feline_fixes(typed, source, limit):
    ...
    """
    # BEGIN PROBLEM 6
    def diff_num(str_a, str_b, i, current_change):
        if abs(len(typed) - len(source)) + current_change > limit:
            return 0
        if i < len(str_a) and i < len(str_b):
            if str_a[i] == str_b[i]:
                return 0 + diff_num(typed, source, i + 1, current_change)
            else:
                return 1 + diff_num(typed, source, i + 1, current_change + 1)     
        else:
            return 0
    a = abs(len(typed) - len(source)) + diff_num(typed, source, 0, 0)
    return  a 
    # END PROBLEM 6
...
def time_per_word(words, timestamps_per_player):
    ...
    for j in range(len(timestamps_per_player)):
        p = [[timestamps_per_player[j][i + 1] - timestamps_per_player[j][i] for i in range(len(timestamps_per_player[0]) - 1)]]
        diff_time += p
...
def fastest_words(match):
    ...
    for i in word_indices:
        p, index_p= time(match, 0, i), 0
        for j in player_indices:
            print("DEBUG:", "j=", j)
            if time(match, j, i) < p:
                p = time(match, j, i)
                index_p = j
        lists[index_p] += [get_word(match, i)]
    return lists
    # END PROBLEM 10
