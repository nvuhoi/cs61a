...
class VendingMachine:
    ...
    'Here is your soda.'
    """
    def __init__(self, commodity, value):
    ...
    def restock(self, stock):
        self.stock += stock
        return f'Current {self.commodity} stock: {self.stock}'
    
    def vend(self):
        if not self.stock:
            return f'Nothing left to vend. Please restock.'
        elif self.fund < self.value:
            return f'Please add ${self.value - self.fund} more funds.'
        elif self.fund == self.value:
            self.stock -= 1
            self.fund = 0
            return f'Here is your {self.commodity}.'
        else:
            change = self.fund - self.value
            self.stock -= 1
            self.fund = 0
...
def store_digits(n):
    ...
    """
    def store_last(n, s=Link.empty):
        if n < 10:
            return Link(n, s)
        s = Link(n % 10, s)
        return store_last(n // 10, s)
    return store_last(n)
...
def two_list(vals, counts):
    ...
    for i in range(len(vals)):
        for _ in range(counts[i]):
            add_list.append(vals[i])
    head = Link.empty
    while add_list:
        head = Link(add_list.pop(), head)
    return head
