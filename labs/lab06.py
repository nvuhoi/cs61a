...
class Account:
    ...
    def withdraw(self, amount):
        ...
        if amount > self.balance:
            before, after = self.balance, self.balance
            self.transactions.append(Transaction(self.id, before, after))
            self.id += 1
...
def make_change(amount, coins):
    ...
    if amount == smallest:
        return [smallest]
    if make_change(amount - smallest, rest) == None:
...
class ChangeMachine:
    ...
    def change(self, coin):
        ...
        change_list = make_change(coin, self.coins)
        if not change_list:
            return [coin]
        for money in change_list:
            self.coins = remove_one(self.coins, money)
        if self.coins.get(coin, 0):
            self.coins[coin] += 1
        else:
            self.coins[coin] = 1
        return change_list
